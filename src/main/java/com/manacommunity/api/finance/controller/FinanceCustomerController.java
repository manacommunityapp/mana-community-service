...
public class FinanceCustomerController {
    ...
    private final LedgerCustomerService service;
    private final PermissionCheckService permissionCheckService;

    @GetMapping
    public ResponseEntity<List<LedgerCustomer>> getAll(@AuthenticationPrincipal UserPrincipal principal) {
        permissionCheckService.requireAnyPermission(principal, VIEW_ADMIN);
        return ResponseEntity.ok(service.getAll());
    }

    @PostMapping
    public ResponseEntity<LedgerCustomer> create(@RequestBody LedgerCustomer body,
    ...
    @PutMapping("/{id}")
    public ResponseEntity<LedgerCustomer> update(@PathVariable Long id, @RequestBody LedgerCustomer body,
                                                 @AuthenticationPrincipal UserPrincipal principal) {
        permissionCheckService.requireAnyPermission(principal, VIEW_ADMIN);
