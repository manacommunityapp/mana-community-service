...
/** Customer credit note (Income → Credit Notes menu). Standalone table. */
@Entity
@Table(name = "finance_credit_note")
@Data
@Builder
@NoArgsConstructor
@AllArgsConstructor
public class FinanceCreditNote extends BaseAuditEntity {

    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    private Long id;

    /** Human-readable code, e.g. CN/001. */
    @Column(length = 24)
    private String code;
    ...
    @ElementCollection
    @CollectionTable(name = "finance_credit_note_line", joinColumns = @JoinColumn(name = "credit_note_id"))
    @Builder.Default
