...
/** Debit note raised against a vendor (Expense → Debit Notes menu). Standalone table. */
@Entity
@Table(name = "finance_debit_note")
@Data
@Builder
@NoArgsConstructor
@AllArgsConstructor
public class FinanceDebitNote extends BaseAuditEntity {

    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    private Long id;

    /** Human-readable code, e.g. DN/001. */
    @Column(length = 24)
    private String code;
    ...
    @ElementCollection
    @CollectionTable(name = "finance_debit_note_line", joinColumns = @JoinColumn(name = "debit_note_id"))
    @Builder.Default
