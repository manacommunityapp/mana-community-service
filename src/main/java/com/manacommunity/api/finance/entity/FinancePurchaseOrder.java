...
/** Purchase order to a vendor (Expense → Purchase Orders menu). Standalone table. */
@Entity
@Table(name = "finance_purchase_order")
@Data
@Builder
@NoArgsConstructor
@AllArgsConstructor
public class FinancePurchaseOrder extends BaseAuditEntity {

    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    private Long id;

    /** Human-readable code, e.g. PO/001. */
    @Column(length = 24)
    private String code;

    /** Draft / Sent / Received / Billed / Cancelled. */
    @Column(length = 30)
    ...
    @ElementCollection
    @CollectionTable(name = "finance_purchase_order_line", joinColumns = @JoinColumn(name = "purchase_order_id"))
    @Builder.Default
