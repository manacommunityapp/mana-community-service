...
public class SportsTournament {
    ...
    // One SportsTournament contains Many Sports Events.
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "community_id")
    @JsonIgnoreProperties({"hibernateLazyInitializer", "handler"})
    private Community community;

    // One SportsTournament has Many Sports Events (1:N). Inverse side — the owning
    ...
    // leave sportsEvents null on builder-created SportsTournament rows.
    //
    ...
    @JoinTable(
        name = "sports_tournament_contact",
