 ...
 */
@Component
public class SportsRandomSeedingStrategy implements SportsSeedingStrategy {
    ...
    public SportsScheduleSequence type() {
        return SportsScheduleSequence.RANDOM;
    }
    ...
    public List<SportsPairing> firstRoundPairings(List<ParticipantRef> players, boolean communityRulesEnabled) {
        ...
        List<SportsPairing> lastAttempt = null;
        for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
            ...
            List<SportsPairing> pairs = SportsSeedingStrategy.neighborPairs(shuffled);
    ...
    private boolean hasConflict(List<SportsPairing> pairs) {
        for (SportsPairing p : pairs) {
     ...
     */
    private void repair(List<SportsPairing> pairs) {
        for (int i = 0; i < pairs.size(); i++) {
            SportsPairing pi = pairs.get(i);
            if (pi.isBye() || !CommunityRules.conflict(pi.home(), pi.away())) continue;
            for (int j = 0; j < pairs.size(); j++) {
                if (j == i) continue;
                SportsPairing pj = pairs.get(j);
                if (pj.isBye()) continue;
                if (!CommunityRules.conflict(pi.home(), pj.away())
                        && !CommunityRules.conflict(pj.home(), pi.away())) {
                    pairs.set(i, new SportsPairing(pi.home(), pj.away()));
                    pairs.set(j, new SportsPairing(pj.home(), pi.away()));
                    break;
                }
            }
        }
