 ...
 * of players. Each concrete strategy corresponds to one {@link SportsScheduleSequence}
 ...
 */
public interface SportsSeedingStrategy {

    /** Which UI option this strategy implements. */
    SportsScheduleSequence type();
     ...
     * @return ordered first-round pairings (a {@link SportsPairing#isBye() BYE} has a null away side)
     */
    List<SportsPairing> firstRoundPairings(List<ParticipantRef> players, boolean communityRulesEnabled);
     ...
     */
    static List<SportsPairing> neighborPairs(List<ParticipantRef> players) {
        List<SportsPairing> pairs = new ArrayList<>();
        int i = 0;
        for (; i + 1 < players.size(); i += 2) {
            pairs.add(new SportsPairing(players.get(i), players.get(i + 1)));
        }
        if (i < players.size()) {
            pairs.add(new SportsPairing(players.get(i), null)); // odd one out → BYE
        }
        return pairs;
    }
}
