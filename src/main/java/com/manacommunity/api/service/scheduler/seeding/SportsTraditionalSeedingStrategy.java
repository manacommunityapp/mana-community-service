 ...
 */
@Component
public class SportsTraditionalSeedingStrategy implements SportsSeedingStrategy {

    @Override
    public SportsScheduleSequence type() {
        return SportsScheduleSequence.TRADITIONAL;
    }

    @Override
    public List<SportsPairing> firstRoundPairings(List<ParticipantRef> seededPlayers, boolean communityRulesEnabled) {
        int n = seededPlayers.size();
        if (n < 2) {
            return SportsSeedingStrategy.neighborPairs(seededPlayers);
        }
        ...
        for (int i = 0; i + 1 < seedOrder.length; i += 2) {
            ...
            pairs.add(new SportsPairing(home, away));
        }
        return pairs;
    }
