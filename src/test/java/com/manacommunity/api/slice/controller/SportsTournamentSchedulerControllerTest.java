...
@WebMvcTest(SportsTournamentSchedulerController.class)
@DisplayName("SportsTournamentSchedulerController")
class SportsTournamentSchedulerControllerTest extends BaseWebMvcTest {

    @MockitoBean SportsTournamentSchedulerService  schedulerService;
    @MockitoBean SportsTournamentConfigRepository  configRepo;
    @MockitoBean SportsTournamentMatchRepository   matchRepo;
    ...
    class BulkSave {
        ...
        private SportsBulkMatchSaveRequest buildRequest(int count) {
            List<SportsBulkMatchSaveRequest.MatchData> matches = new java.util.ArrayList<>();
            for (int i = 1; i <= count; i++) {
                matches.add(new SportsBulkMatchSaveRequest.MatchData(
                        10L, 84L, "Group A", "GROUP", i,
                        "Team A", "1", "Team B", "2",
                        LocalDate.now().toString(), "10:00", 90, 1L, 1L, "SCHEDULED"));
            }
            return new SportsBulkMatchSaveRequest(matches);
        }
    ...
    class GeneratePlayoff {
        ...
        private SportsPlayoffGenerateRequest input() {
        ...
        void admin_returns200() throws Exception {
            when(playoffGenerator.buildPlayoffBracket(any())).thenReturn(List.of(
                    new SportsPlayoffMatchDraftResponse("playoff-final", "Final", "FINAL", 1,
                            new SportsPlayoffMatchDraftResponse.ParticipantRef("a", "A"),
                            new SportsPlayoffMatchDraftResponse.ParticipantRef("b", "B"),
